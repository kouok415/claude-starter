...
2. **計畫 fusion** —— 3 個不同視角的 `planner` → 紅隊 `plan-critic` →
   綜合出小里程碑的 `plan.md`,每個里程碑帶一條 `verify:` 指令。
3. **里程碑迴圈** —— 每個里程碑用全新的 `executor` context 執行、
   對抗式 `verifier` 驗收、過關即 commit,每三關做一次漂移巡檢。
4. **Stop gate** —— 回合想結束時 `stop-gate.sh` 重跑當前里程碑的
   verify;紅燈就結束不了。Definition of done 從散文變成物理法則。
5. **升級梯** —— 換方法重試 → 3 個分歧策略平行(worktree)→
   `reframer` 改寫問題本身 → 停下回報(三振規則)。

狀態放在 `.ai_context/tasks/<slug>/`(`spec.md`、`plan.md`、
`lessons.md`),`/clear` 和壓縮後會自動重新注入,run 不怕 context
流失。完結時 `/wrap` 把記分板(profile、閘門失敗數、用到第幾級升級梯)
歸檔到 `journal/` —— 用數據判斷這套 harness 值不值得。
...
│   ├── agents/                 /task 班底:planner、plan-critic、
