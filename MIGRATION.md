...
- **[-7. claude-starter v3.14 → v3.14.2](#-7-claude-starter-v314--v3142)** — the
...
## -7. claude-starter v3.14 → v3.14.2

Two patch releases on one finding. v3.14.1 exempted the session
...
`permissions.ask` rule `Bash(rm -rf:*)`, which the CLI honors in every
permission mode — `bypassPermissions` included — matched per subcommand
(`cd $S && rm -rf x`, `rm -rf $S/x`) and regardless of what a PreToolUse
hook returns. No hook-side exemption can reach a declarative ask rule.
...
ask rules (`git push`, `git reset --hard`, `git clean`, `sudo`) are
unchanged — they prompt under bypass by design.

### Migrating

- Stock `settings.json`: `sync-project.sh --update-stock <project>`
  replaces it. Customized copies: delete the `"Bash(rm -rf:*)"` entry
  from `permissions.ask` by hand.
...
time. During a /task run the human used to see only harness verdicts
("M2 PASS"); what was being built, which judgment calls were made, and
where the run stood lived in subagent contexts and died there. Narration
duties that rest on model diligence erode under token pressure, so v3.12
splits the human channel into a machine-generated skeleton and a minimal,
nagged model remainder.

### What changed, and why

| v3.11 | v3.12 | Reason |
|---|---|---|
| milestone reports were gate verdicts — one bit; the human could neither follow what was being built nor veto a wrong turn before later milestones compounded it | plan format gains a `- demo:` line per milestone (the verify translated to "run X, you will see Y"); executors return the demo as REAL output plus their `[ASSUMED: ...]` interpretation calls (≤3); PASS reports are demo-first, never a bare verdict; a kickoff brief before M1 states what will exist, every demo promise, and the veto contract (one milestone = one commit = cheap rollback until merge) | showing beats describing — real output cannot be narrated over; the promise→demo chain turns plan approval into approving observable claims, and named rollback points convert "I must catch it live" anxiety into cheap after-the-fact review |
| nothing answered "where are we and what is happening" mid-run; transcript narration scrolls away | `tasks/<slug>/STATUS.md`: machine region (position, mermaid map, demo promises, change footprint, last verify tail, `[ASSUMED]` ledger) regenerated by `scripts/task-status.sh` on every real verify run (stop-gate) and resume (session-start); the model owns only §Architecture, with a once-per-session PASS-time `systemMessage` nag while it is still a stub; gitignored like `.gate-cache`; opt-in statusline `scripts/task-statusline.sh` | a live view must not depend on model diligence: everything derivable from plan/spec/gatelog is derived mechanically and self-heals; only what genuinely requires the model (the architecture prose) is left to it — and that residue is nagged, not trusted |
| `[ASSUMED]` existed only at intake; drift checks fired only after `risk: high` and on the L cadence — an all-low/med M task got zero end-to-end; ACs lock function, never direction | spec §Assumptions is the run-long decision ledger (the one section append-only after the freeze; executors report calls, the orchestrator appends); drift check also fires at the M midpoint; the verifier's drift mode judges intent (Goal, Constraints, Out of scope) and audits the ledger for human-vetoable calls; intake always asks the direction question ("what would make you reject this even with every AC green?") | direction drifts in judgment calls, not diffs: only a recorded call can be vetoed, only recorded intent can anchor a drift verdict, and the milestone-boundary veto moments are the value instrument — zero vetoes over many tasks means the mechanism is dead weight (measure before trusting, ADR-017's lesson) |
| task SKILL.md size guard at 11 KB, 166 B headroom | guard at 12 KB (L1-6); the file lands at ~12.1 KB after compensating trims | the new duties are orchestrator-side and must ride the hot path; gutting incident-honed protocol text to fit a round number would trade correctness for cosmetics |

New files: `scripts/task-status.sh`, `scripts/task-statusline.sh`; new
gitignore lines for `tasks/*/STATUS.md` + `tasks/*/.last-verify`; CI
section `L1-19`. Sync via `sync-project.sh --update-stock` picks all of
it up; projects without the new scripts degrade silently (the gate guards
every call).
...
| `settings.json` carried `Write(...)` deny rules for gatelog/spawnlog alongside the `Edit(...)` ones — Claude Code matches only `Edit(path)` for file edits and printed a two-line warning about the dead pair in **every** session | the dead `Write(...)` rules are removed; `Edit(...)` covers every file-editing tool (Write included), so protection is unchanged and the per-session warning noise is gone | found by the v3.11 e2e batch: the platform's own warning text names the fix; a dead rule that generates noise trains readers to ignore the warning channel |
...
| after the first integrity interruption of a session, later distinct dark-gate states ended the turn silently with no gatelog row; a stuck milestone appended a fresh `STUCK` row on every stop past the third | log-always / interrupt-once: every integrity detection appends its `INTEGRITY` row (the session marker gates only the stderr interrupt); one `STUCK` row per (session, milestone) — a `PASS` re-arms it, the `systemMessage` keeps repeating | ADR-007's "quiet gatelog provably means clean run" and the v3.9 counted-yield tallies the human reads were both false under the old logging |
