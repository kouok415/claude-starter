...
| "Catastrophic bash never runs unattended" (never force-push, no sudo) | `bash-guard.sh` PreToolUse tripwire: force-push in any spelling (flags or `+refspec`) / `sudo` / root deletes (incl. `--no-preserve-root`, v3.10) denied and logged to `private/bash-guard.log`; `rm -rf` beyond the project tree (absolute, `~/`, `$HOME/`, `..` — v3.14.3) and `.env` access downgraded to a confirmation — the session's own `/tmp/claude-*/…/scratchpad` tree is exempt (v3.14.1: the harness discards it anyway, and a confirmation that fires every cleanup only teaches click-through). The hook is the ONLY `rm` tier: v3.14.2 dropped the `Bash(rm -rf:*)` ask rule from `settings.json`, because the CLI honors declarative `ask` rules in every mode — bypass included — per subcommand and regardless of hook output, so that rule kept prompting on every scratchpad cleanup and no hook-side exemption could reach it (the CLI's built-in critical-path breaker still asks for root / top-level / home / cwd deletes in every mode). Declarative `ask` (push, reset --hard, clean, sudo) and `deny` rules stay in `settings.json` for what should prompt even under bypass. Contains-matching beats prefix rules — but it is a tripwire, not a sandbox |
