...
| "A block must survive the host CLI, not just the hook" (v3.14) | every exit-2 path prints one fixed stdout marker line first — the CLI's missing-hook heuristic (`exit 2` + empty stdout + stderr matching `/no such file\|can't open/i`, observed through 2.1.229) otherwise demotes legitimate blocks to non-blocking; a just-armed milestone's verify output is exactly that bait. L1-21 exercises every block path |
| "An in-flight executor is a wake guarantee, not a red state" (v3.14) | fresh executor spawn row for the armed milestone → the gate defers the verify once per spawn row (`WAITING` gatelog row + systemMessage, quiet stop) — the completion notification re-wakes the session; past the 45-min freshness window a hung executor stops shielding the milestone |
| "A green milestone boundary is an explicit decision, never a silent park" (v3.14) | milestone PASS with `[pending]` left → ONE continuation prompt per (session, milestone): continue, or pause explicitly (unanswered human questions must be restated, never crossed); the stop after it passes and records a `PAUSED` row, so "waiting on the human" is legible instead of indistinguishable from a stall |
| "'Wait for me here' is mechanical, not model judgment" (v3.14) | `- checkpoint: human` on a milestone: its PASS holds the run (PAUSED + repeating systemMessage) until `tasks/<slug>/ack-<id>` exists; ack files are Edit-denied and bash-guard-confirmed (the confirmation IS the sign-off), and work advanced past an un-acked checkpoint is an integrity block. Tripwire class, like gatelog |
...
| "The re-anchor pays for the current milestone, not the whole plan" | session-start injects a filtered plan.md view — header + all headings + verify/risk lines + the full `[in_progress]` section; self-declares as filtered, warns past 8 KB. Forward constraints live in the *target* milestone's comment block (SKILL rule), so they surface exactly when it arms instead of renting context every session. Executors get the armed milestone's full section IN the spawn prompt and never re-read raw plan.md; the raw file itself warns past 16 KB, and an all-`[done]` task with `CURRENT` still present nudges toward `/wrap` instead of silently re-paying the task attachment forever (v3.11) |
...
| "The human can always see where the run stands and what it is building" (v3.12) | `tasks/<slug>/STATUS.md` live view: the machine region (position, mermaid milestone map, demo promises, change footprint, last verify output, `[ASSUMED]` ledger) is regenerated by `scripts/task-status.sh` — the stop-gate re-renders it after every real verify run, session-start on every resume — and the model owns only §Architecture (a once-per-session `systemMessage` nags while it is still the stub). Gitignored like `.gate-cache`, so refreshing it can never evict the PASS-cache. Opt-in one-line statusline: `scripts/task-statusline.sh` |
| "Milestones are demos and vetoable decisions, never bare verdicts" (v3.12) | plan format carries a `- demo:` line per milestone (the verify translated to "run X, you will see Y"); the kickoff brief before M1 states what will exist, every demo promise, and the veto contract (one milestone = one commit = cheap rollback until merge); executors return the demo as real output plus `[ASSUMED: ...]` interpretation calls (≤3) that land in spec §Assumptions — the run-long append-only decision ledger; PASS reports are demo-first; the verifier's drift mode (also at the M midpoint now) judges intent — Goal, Constraints, Out of scope — and names ledger entries the human would plausibly veto |
...
| "Catastrophic bash never runs unattended" (never force-push, no sudo) | `bash-guard.sh` PreToolUse tripwire: force-push in any spelling (flags or `+refspec`) / `sudo` / root deletes (incl. `--no-preserve-root`, v3.10) denied and logged to `private/bash-guard.log`; `rm -rf` beyond the project tree (absolute, `~/`, `$HOME/`, `..` — v3.14.3) and `.env` access downgraded to a confirmation — the session's own `/tmp/claude-*/…/scratchpad` tree is exempt (v3.14.1: the harness discards it anyway, and a confirmation that fires every cleanup only teaches click-through). The hook is the ONLY `rm` tier: v3.14.2 dropped the `Bash(rm -rf:*)` ask rule from `settings.json`, because the CLI honors declarative `ask` rules in every mode — bypass included — per subcommand and regardless of hook output, so that rule kept prompting on every scratchpad cleanup and no hook-side exemption could reach it (the CLI's built-in critical-path breaker still asks for root / top-level / home / cwd deletes in every mode). Declarative `ask` (push, reset --hard, clean, sudo) and `deny` rules stay in `settings.json` for what should prompt even under bypass — and v3.14.3 adds three there for `rm -rf ~/*`, `$HOME/*`, `../*`: the CLI drops hook asks under bypass, and these spellings never touch a `/tmp` scratchpad, so they can prompt there without the storm; the hook mirrors them (any flag spelling) for the other modes and the stop-gate denylist. v3.14.4: the ask tier reads the command with heredoc bodies removed — prose in a memory-file write is not a command (17 of the fleet's 48 subagent `.env` stalls were exactly that, a median 10 minutes each) — and treats the project's own tree like the scratchpad for `rm -rf`; the deny tier still reads the full text. v3.14.5: any `/tmp/<name>…` passes too — on a dev box `/tmp` is disposable by definition, and every `/tmp/<name>` the fleet's subagents deleted was their own — while `/tmp`, `/tmp/`, `/tmp/*` still ask (`/tmp/claude-<uid>` as a whole passes: known edge). Contains-matching beats prefix rules — but it is a tripwire, not a sandbox |
